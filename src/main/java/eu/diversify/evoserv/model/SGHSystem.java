package eu.diversify.evoserv.model;


import eu.diversify.evoserv.SGHModel;
import eu.diversify.evoserv.utils.DataExportUtils;
import eu.diversify.evoserv.utils.collections.Population;
import eu.diversify.evoserv.random.RandomUtils;
...
public class SGHSystem {
    ...
    public static SGHSystem generateSGHSystem(int max_clients, int max_servers, boolean min_redundancy_links) {
        ...
        if (min_redundancy_links) result.createLinksWithGreedyMinimmizeRedundancy();
        else result.createRandomLinks();
    ...
    public static SGHSystem generateRealisticManualSGHSystem(int max_clients, int max_servers, boolean min_redundancy_links) {
    ...
    public ArrayList<SGHClientApp> clients;
    public ArrayList<SGHServer> servers;
    ...
    private void createLinksWithGreedyMinimmizeRedundancy() {
        // Clone the server list to randomize its order for each client
        ArrayList<SGHServer> _servers = (ArrayList<SGHServer>)servers.clone();
        // For each client, add a set of links to satistfy its dependencies
        ...
        for (SGHClientApp c : clients) {
            ...
            ArrayList reqs_not_yet_covered = c.getRequests();
            ArrayList reqs_already_covered = new ArrayList<SGHRequest>();
             
            while (!reqs_not_yet_covered.isEmpty()) {
            
                int min = 0;
                int nb_req = 0;
                SGHServer server_which_minimize = null;

                for (SGHServer s : _servers) {
                    if (!s.hasCapacity()) continue;

                    if (s.countHowManyRequestServerCanHandle(reqs_not_yet_covered) > 0) {
                        if (server_which_minimize == null) {
                            server_which_minimize = s;
                            min = s.countHowManyRequestServerCanHandle(reqs_already_covered);
                            nb_req = s.countHowManyRequestServerCanHandle(reqs_not_yet_covered);
                            //System.out.println("      MIN : " + min);
                        }
                        //System.out.println("Already Covered : " + s.countHowManyRequestServerCanHandle(reqs_already_covered));
                        if (s.countHowManyRequestServerCanHandle(reqs_already_covered) < min) {
                            server_which_minimize = s;
                            min = s.countHowManyRequestServerCanHandle(reqs_already_covered);
                            nb_req = s.countHowManyRequestServerCanHandle(reqs_not_yet_covered);
                            //System.out.println("LOWER MIN : " + min);
                        }
                        if (s.countHowManyRequestServerCanHandle(reqs_already_covered) == min && nb_req > s.countHowManyRequestServerCanHandle(reqs_not_yet_covered)) {
                            server_which_minimize = s;
                            min = s.countHowManyRequestServerCanHandle(reqs_already_covered);
                            nb_req = s.countHowManyRequestServerCanHandle(reqs_not_yet_covered);
                        }
                ...
                if (server_which_minimize != null) {
                    reqs_already_covered.addAll(server_which_minimize.getRequestsWhichCanHandle(reqs_not_yet_covered));
                    reqs_not_yet_covered = server_which_minimize.filterRequestsWhichCanHandle(reqs_not_yet_covered);
                    c.getLinks().add(server_which_minimize);
                    server_which_minimize.increaseLoad();
                }
                else {
                    // We give up!
                    for (SGHServer s : c.getLinks()) {
    ...
    public int[] distLinksUtility() {
        int biggest_client = 0;
        for(SGHClientApp c : clients) if (c.getRequests().size()>biggest_client) biggest_client = c.getRequests().size();
        
        int[] result = new int[biggest_client+1];
        
        for(SGHClientApp c : clients) {
            for (SGHServer s : c.links) {
                int link_utility = 0;
                for(SGHRequest r : c.getRequests()) {
                    if (s.canHandle(r)) link_utility++;
                }
                result[link_utility]++;
            }
        }
    ...
    public int getTotalLinkUtility() {
        int link_utility = 0;
        for(SGHClientApp c : clients) {
            for (SGHServer s : c.links) {
                for(SGHRequest r : c.getRequests()) {
     ...
     public int getTotalRequestRedundancyWithValueTwo() {
        int most_links = 0;
        for(SGHClientApp c : clients) if (c.getLinks().size()>most_links) most_links = c.getLinks().size();
        
        int[] result = new int[most_links+1];
        
        for(SGHClientApp c : clients) {
            for(SGHRequest r : c.getRequests()) {
                int req_redundancy = 0;
                for (SGHServer s : c.links) {
                    if (s.canHandle(r)) req_redundancy++;
                }
                result[req_redundancy]++;
            }
        }
        return result[2];
    }
    
    public int[] distRequestRedundancy() {
        int most_links = 0;
        for(SGHClientApp c : clients) if (c.getLinks().size()>most_links) most_links = c.getLinks().size();
    ...
    public void exportGraphStatistics(File folder) {
        ...
        DataExportUtils.writeGNUPlotScriptForIntArray(distRequestRedundancy(), folder, "dist_requests_redundancy", " Redundancy", "# Requests");
        DataExportUtils.writeGNUPlotScriptForIntArray(distLinksUtility(), folder, "dist_links_utiliy", "Utility", "# Links");
    }
