...
public class SGHServer extends SGHNode {
    ...
    public ArrayList<SGHRequest> getRequestsWhichCanHandle(ArrayList<SGHRequest> reqs) {
        ArrayList<SGHRequest> result = new ArrayList<SGHRequest>();
        for (SGHRequest r : reqs) {
            if (canHandle(r)) {
                result.add(r);
    ...
    public int countHowManyRequestServerCanHandle(ArrayList<SGHRequest> reqs) {
        int result = 0;
        for (SGHRequest r : reqs) {
            if (canHandle(r)) result++;
        }
        return result;
    }
    ...
    public void removeFeature(SGHFeature f) {
        featureSet.remove(f);
        features.get(f.getVariationPoint()).remove(f);
    }
    
    public void addFeature(SGHFeature f) {
        featureSet.add(f);
