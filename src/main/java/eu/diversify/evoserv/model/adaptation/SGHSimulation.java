...
package eu.diversify.evoserv.model.adaptation;


import eu.diversify.evoserv.model.SGHClientApp;
import eu.diversify.evoserv.model.SGHExtinctionSequence;
import eu.diversify.evoserv.model.SGHServer;
import eu.diversify.evoserv.model.SGHSystem;
import eu.diversify.evoserv.model.adaptation.SGHClientAdaptation;
import eu.diversify.evoserv.model.adaptation.SGHServerAdaptation;
...
public class SGHSimulation {
    ...
    public SGHSystem system;
    public ArrayList<SGHSimulationStep> steps;
    ...
    private void initialize_simulation() {
        ...
        if (compute_extinctions) {
            ...
            step.total_utility = system.getTotalLinkUtility();
            step.total_redundancy = system.getTotalRequestRedundancyWithValueTwo();
        }
        steps.add(step);
        System.out.println(step.toString());
    }
    ...
    public void exportRobustnessData(String name, File folder) {
        ...
        int[] utility = new int[steps.size()];
        int[] redundancy = new int[steps.size()];
        for(int i=0; i<steps.size(); i++) {
            robustness[i] = steps.get(i).robustness*100;
            utility[i] = steps.get(i).total_utility;
            redundancy[i] = steps.get(i).total_redundancy;
        }
        DataExportUtils.writeGNUPlotScriptForDouble(robustness, folder, name);
        
        DataExportUtils.writeGNUPlotScriptForUtility(utility, folder, name);
        
        DataExportUtils.writeGNUPlotScriptForRedundancy(redundancy, folder, name);
    }
